...
impl Context {
    ...
    #[cfg(target_arch = "wasm32")]
    pub fn new_wasm(table: Option<String>, port: Option<u32>) -> Context {
        Context {
            config: None,
            cache: Some(Cache::default()),
            overwritten_region: Some(Region::from_static(LOCAL_REGION)),
            overwritten_table_name: table,
            overwritten_port: Some(port.unwrap_or(8000)),
            output: None,
            should_strict_for_query: None,
            retry: None,
        }
    ...
    async fn build_sdk_config(
        ...
        {
            let _ = retry_config;
            return crate::wasm_sdk_config(region_name);
        }
        #[cfg(not(target_arch = "wasm32"))]
        {
...
#[cfg(target_arch = "wasm32")]
fn write_dynein_file(_file_type: DyneinFileType, _content: String) -> Result<(), DyneinConfigError> {
    Ok(()) // no filesystem in the browser; schema cache is not persisted
}
